# TinyGPS + Primary Frequency Regulation Monitor Example

## Overview

This project demonstrates how to use Primary Frequency Regulation Monitor to visualize GPS location data from a GPS module (GY-NEO6MV2) connected to an Arduino Mega 2560. The Arduino program reads GPS data (latitude, longitude, and altitude) and sends it to Primary Frequency Regulation Monitor via serial communication for display on a map.

![GPS Data in Primary Frequency Regulation Monitor](doc/screenshot.png)
...
To visualize the data, Primary Frequency Regulation Monitor needs to be properly configured to parse the incoming GPS data:
...
2. **Open Primary Frequency Regulation Monitor and Import `TinyGPS.json`**:

   Launch Primary Frequency Regulation Monitor and load the `TinyGPS.json` file included in this project using the **Project Editor**. This file contains all necessary configurations for interpreting the data transmitted by the Arduino.

#### To Create the Project from Scratch:

Set up Primary Frequency Regulation Monitor to correctly interpret the GPS data by configuring the following in a new project:

- **Frame Start Sequence**: `$`
- **Frame End Sequence**: `;`
- **Data Separator**: `,`

This setup informs Primary Frequency Regulation Monitor how to parse the data being sent by the Arduino. 

Then, click on the **Map** button within Primary Frequency Regulation Monitor to create a GPS map widget automatically. The widget will use the latitude and longitude data to show the current position, while the altitude data can be logged for later analysis, which can be found in CSV logs generated by Primary Frequency Regulation Monitor.
...
### 4. Viewing GPS Data in Primary Frequency Regulation Monitor
...
- **No Data on Map**: Ensure that the GPS module has a clear view of the sky to get satellite fixes. Verify that the correct port and baud rate are selected in Primary Frequency Regulation Monitor.
